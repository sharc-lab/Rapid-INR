...
parser.add_argument("-dp", "--dataset_path", help="The path of the dataset",
                    default=f"/export/hdd/scratch/dataset/mini_imagenet_split/train/")
parser.add_argument("-ld", "--logdir", help="Path to save logs",
                    default=f"./mini_ImageNet_logfiles")
parser.add_argument("-wd", "--INR_weights_dir", help="Path to save INR weights",
                    default=f"./weights_mini_ImageNet/trail")
parser.add_argument("-ri", "--resized_image",
                    help="Path to save resized images", default=f"./resizeimg_mini_imagenet")
parser.add_argument("-rconi", "--reconstructed_image",
                    help="Path to save reconstructed images", default=f"./reconimages_mini_ImageNet/trial")
parser.add_argument("-ra", "--reconstructed_image_array",
                    help="Path to save reconstructed images array", default=f"./mini_ImageNet_array")
...
parser.add_argument("-device", "--device",
                    help="The id of the GPU", default=f"cuda:0")

parser.add_argument("-nit", "--num_iters",
                    help="Number of iterations to train for", type=int, default=2000)
parser.add_argument("-nim", "--num_images",
                    help="Number of images to train on", type=int, default=7000)
parser.add_argument("-lr", "--learning_rate",
                    help="Learning rate", type=float, default=2e-3)
...
parser.add_argument("-bs", "--bs_number",
                    help="the index of the image to training.", type=int, default=0)
...
# Compares 'filename' extension to common image file types.
...
class Trainer():
    def __init__(self, representation, lr=1e-3, print_freq=1):
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer, T_max=num_iters)
...
for i in range(train_image_num):
    ...
    torch.save(trainer.best_model, logdir + dir_appendix +
               f'/best_model_{start_idx + i}.pt')
    ...
    with torch.no_grad():
        ...
        np.save(logdir_array +
                f'/fp_reconstruction_{start_idx + i}.npy', img_array)

        if not os.path.exists(log_dir_image + dir_appendix):
            os.makedirs(log_dir_image + dir_appendix)
        save_image_new(img_recon, log_dir_image + dir_appendix +
                       f'/fp_reconstruction_{start_idx + i}.png')
    #print(f'Best training psnr: {trainer.best_vals["psnr"]:.2f}')
