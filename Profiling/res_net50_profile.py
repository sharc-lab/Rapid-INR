...
def train(epoch, coordinates, linear_0_weight, linear_0_bias, linear_1_weight,
    ...
    for batch_idx in range(num_inter):
        ...
        output = decode(0, coordinates, linear_0_weight, linear_0_bias,
                        ...
                        linear_4_weight, linear_4_bias)
        dec_t = time.time() - start_time
        dec_time.append(dec_t)
        #-------------------------------------------------------------
        aug_s = time.time()
        #print(linear_0_weight.requires_grad)
        reconstructed_augment = transform_train(output)
        aug_t = time.time() - aug_s
        aug_time.append(aug_t)
        #-------------------------------------------------------------
        train_s = time.time()
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, start_epoch + 35):
        ...
        train(epoch, coordinates, linear_0_weight, linear_0_bias,
              ...
              train_label_tensor)
        print("decoding time: %s" % str(sum(dec_time)))
        print("aug time: %s" % str(sum(aug_time)))
        print("training time: %s" % str(sum(train_time)))
